...
$aset['jobslen']=isset($aset['jobslen'])?intval($aset['jobslen']):14;
...
//企业类型
if (isset($aset['companytype']) && $aset['companytype']<>'')
{
	$wheresql.=" AND companytype=".intval($aset['companytype']);
}
...
if (isset($aset['page']))
{
	...
	//echo "SELECT * FROM ".table('jobs')." ".$wheresql.$orderbysql;
		...
		$row['refreshtime_cn']=daterange(time(),$row['refreshtime'],'Y-m-d',"");
