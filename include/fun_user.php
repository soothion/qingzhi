...
function user_register($username,$password,$member_type=0,$email,$uc_reg=true)
{
	...
	$insert_id=inserttable(table('members'),$setsqlarr,true);
				...
				if(!$db->query("INSERT INTO ".table('members_setmeal')." (uid) VALUES ('{$insert_id}')")) return false;
					$points=get_cache('points_rule');
							if ($points['reg_points']['value']>0)
							{
								...
								write_memberslog($insert_id,1,9001,$username,"新注册会员,({$operator}{$points['reg_points']['value']})");
							}
			...
			if($member_type=="2")
			{
				if(!$db->query("INSERT INTO ".table('members_points')." (uid) VALUES ('{$insert_id}')"))  return false;
					$points=get_cache('points_rule');
							if ($points['reg_user']['value']>0)
							{
								include_once(QISHI_ROOT_PATH.'include/fun_company.php');
								report_deal($insert_id,$points['reg_user']['type'],$points['reg_user']['value']);
								$operator=$points['reg_points']['type']=="1"?"+":"-";
								write_memberslog($insert_id,2,9001,$username,"新注册会员,({$operator}{$points['reg_user']['value']})");
							}
			}
	...
	if ($record)
	{
		...
		if ($_SESSION['utype']=="2")
		{
			...
			if ($rule['user_login']['value']>0 )
			{
				...
				$db->query("INSERT INTO ".table('members_handsel')." (uid,htype,addtime) VALUES ('{$_SESSION['uid']}', 'userlogin','{$time}')");
				require_once(QISHI_ROOT_PATH.'include/fun_company.php');
				report_deal($_SESSION['uid'],$rule['user_login']['type'],$rule['user_login']['value']);
				$user_points=get_user_points($_SESSION['uid']);
				$operator=$rule['user_login']['type']=="1"?"+":"-";
				$_SESSION['handsel_userlogin']=$operator.$rule['user_login']['value'];
				write_memberslog($_SESSION['uid'],2,9001,$_SESSION['username'],date("Y-m-d")." 第一次登录，({$operator}{$rule['user_login']['value']})，(剩余:{$user_points})");
				}
			}
		}
