...
function CopyItem(showid) {
	var htmlstr='<div style=\"height:28px;\">&nbsp;&nbsp;已经选择分类：<span class=\"empty\">[清空已选]</span></div>';
	$(showid + " .item :checkbox[checked][class='b']").each(function(index) {
		if($(this).attr("rel")){
			rel="id="+$(this).attr("rel")+" class=\"s\"";
		}else{
			rel="class=\"b\"";
		}
		htmlstr += "<label><input type=\"checkbox\" "+rel+" value=\"" + $(this).attr("value") + "\" title=\"" + $(this).attr("title") + "\" checked/>" + $(this).attr("title") + "</label>";
	})
	...
	$(showid + " .OpenFloatBox .item label :checkbox[checked]").parent().css("color", "#ff9822");
	$(showid + " .OpenFloatBox .sitem :checkbox[checked]").each(function(index) {
		$(this).parent().parent().prev().css("color", "#ff9822");
	});
	SetBoxBg(showid);
	//清空
