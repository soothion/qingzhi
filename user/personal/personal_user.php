...
elseif ($act=='avatars_save')
{
	...
	if ($rule['upload_photo']['value']>0)
	{
		$time=time();
		$info=$db->getone("SELECT uid FROM ".table('members_handsel')." WHERE uid ='{$_SESSION['uid']}' AND htype='upload_photo' LIMIT 1");
		if(empty($info))
		{				
			$db->query("INSERT INTO ".table('members_handsel')." (uid,htype,addtime) VALUES ('{$_SESSION['uid']}', 'upload_photo','{$time}')");
			...
			report_deal($_SESSION['uid'],$rule['upload_photo']['type'],$rule['upload_photo']['value']);
			$user_points=get_user_points($_SESSION['uid']);
			$operator=$rule['upload_photo']['type']=="1"?"+":"-";
			write_memberslog($_SESSION['uid'],2,9001,$_SESSION['username'],date("Y-m-d")." 第一次上传头像，({$operator}{$rule['upload_photo']['value']})，(剩余:{$user_points})");
		}
	}
	//积分计算结束
