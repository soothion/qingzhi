...
elseif ($act=='setmeal_report' && $_CFG['operation_mode']=="2")
{
	...
	$smarty->assign('title','服务消费明细 - 个人会员中心 - '.$_CFG['site_name']);
	$smarty->assign('act',$act);
	$smarty->assign('report',get_user_report($offset, $perpage,$wheresql));
	$smarty->assign('page',$page->show(3));
	$smarty->display('member_personal/personal_setmeal_report.htm');
}
...
elseif ($act=='points_rule')
{
	...
	$smarty->display('member_personal/personal_points_rule.htm');
}
...
{
	$smarty->assign('title','服务列表 - 个人会员中心 - '.$_CFG['site_name']);
	$smarty->assign('setmeal',get_setmeal());
	$smarty->display('member_personal/personal_setmeal_list.htm');
}
