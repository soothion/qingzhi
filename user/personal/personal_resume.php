...
elseif ($act=='refresh')
{
	if(refresh_resume($_SESSION['uid'])){
				//扣除积分
				$points=get_cache('points_rule');
				if ($points['resume_refresh']['value']>0)
				{	
					...
					report_deal($_SESSION['uid'],$points['resume_refresh']['type'],$points['resume_refresh']['value']);
					$operator=$points['resume_refresh']['type']=="1"?"+":"-";
					write_memberslog($_SESSION['uid'],1,9001,$_SESSION['username'],"刷新简历,({$operator}{$points['resume_refresh']['value']})");
				}
				showmsg('操作成功！',2);
			}
	else showmsg('操作失败！',0);
...
elseif ($act=='make1')
{
	...
	//total[2]>=intval($_CFG['resume_max'])
	$smarty->assign('resume_max',intval($_CFG['resume_max']));
...
elseif ($act=='make1_save')
{
	...
	$setsqlarr['aboutme']=trim($_POST['aboutme']);
...
elseif ($act=='make3_save')
{
	...
	foreach ($title as $key => $value) {
		if($value=='') showmsg('请填写技能名称！',1);
		if($level[$key]<1||$level[$key]>7) showmsg('填写1-7之间的整数等级！',1);
		else{
			$temp.=$value.','.$level[$key].'|';
		}
	}
	$setsqlarrspecialty['specialty']=rtrim($temp,"|");
...
elseif ($act=='save_resume_photo_cutting')
{
	...
	$imgresize->cut(250,250, intval($posary[0]), intval($posary[1]));
...
elseif ($act=='addcomplete')
{
	...
	{	
		...
		report_deal($_SESSION['uid'],$points['resume_add']['type'],$points['resume_add']['value']);
		$operator=$points['resume_add']['type']=="1"?"+":"-";
		write_memberslog($_SESSION['uid'],1,9001,$_SESSION['username'],"创建简历,({$operator}{$points['resume_add']['value']})");
	}
	showmsg("操作完成！",2,$link);
}
