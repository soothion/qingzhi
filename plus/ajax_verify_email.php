			...
			else
			{
					...
					if ($_SESSION['utype']=="2")
					{
						...
						if ($rule['verify_email']['value']>0)
						{
							...
							$db->query("INSERT INTO ".table('members_handsel')." (uid,htype,addtime) VALUES ('{$_SESSION['uid']}', 'verifyemail','{$time}')");
							require_once(QISHI_ROOT_PATH.'include/fun_company.php');
							report_deal($_SESSION['uid'],$rule['verify_email']['type'],$rule['verify_email']['value']);
							$user_points=get_user_points($_SESSION['uid']);
							$operator=$rule['verify_email']['type']=="1"?"+":"-";
							$_SESSION['handsel_verifyemail']=$_CFG['points_byname'].$operator.$rule['verify_email']['value'];
							write_memberslog($_SESSION['uid'],1,9001,$_SESSION['username']," 邮箱通过验证，{$_CFG['points_byname']}({$operator}{$rule['verify_email']['value']})，(剩余:{$user_points})");
							}
						}
					}
					exit("success");
