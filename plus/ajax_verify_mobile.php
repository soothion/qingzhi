			...
			else
			{
					...
					if ($_SESSION['utype']=="2")
					{
						...
						if ($rule['verify_mobile']['value']>0)
						{
							...
							$db->query("INSERT INTO ".table('members_handsel')." (uid,htype,addtime) VALUES ('{$_SESSION['uid']}', 'verifymobile','{$time}')");
							require_once(QISHI_ROOT_PATH.'include/fun_company.php');
							report_deal($_SESSION['uid'],$rule['verifymobile']['type'],$rule['verify_mobile']['value']);
							$user_points=get_user_points($_SESSION['uid']);
							$operator=$rule['verify_mobile']['type']=="1"?"+":"-";
							$_SESSION['handsel_verifymobile']=$_CFG['points_byname'].$operator.$rule['verify_mobile']['value'];
							write_memberslog($_SESSION['uid'],1,9001,$_SESSION['username']," 手机通过验证，{$_CFG['points_byname']}({$operator}{$rule['verify_mobile']['value']})，(剩余:{$user_points})");
							}
						}
					}
					exit("success");
